...
- [Development guidelines](#development-guidelines)
    - [General SecureCloud2 Core](#general-securecloud2-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
SecureCloud2 Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play SC2s (tSC2)" on the test network, if you
...
SecureCloud2 Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/securecloud2-project/securecloud2/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_securecloud2
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_securecloud2 --log_level=test_suite
$ valgrind -v --leak-check=full src/securecloud2d -printtoconsole
```
...
# A coverage report will now be accessible at `./test_securecloud2.coverage/index.html`.
```
...
- BitcoinMiner : Generates SC2s (if wallet is enabled).
...
However, in open source software such as SecureCloud2 Core, where everyone uses
...
pay attention to for reviewers of SecureCloud2 Core code.

General SecureCloud2 Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: SecureCloud2 Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against SecureCloud2 Core so that they can be integrated
...
file descriptors it will cause problems with SecureCloud2's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:SecureCloud2-Project/SecureCloud2.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `securecloud2-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `securecloud2-cli` and the GUI debug console use this table to determine how to
