...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nCreates raw zSC2 coinstakes (without MN output).\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zSC2 denom)
    ...
    if (!pwalletMain->CreateZSC2OutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zsc2 output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZSC2OutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zsc2 output");
    }
