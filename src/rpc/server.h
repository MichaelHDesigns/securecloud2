...
/**
 * SecureCloud2 RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZsc2Spend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzsc2seed(const UniValue& params, bool fHelp);
extern UniValue getzsc2seed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzsc2(const UniValue& params, bool fHelp);
extern UniValue dzsc2state(const UniValue& params, bool fHelp);
