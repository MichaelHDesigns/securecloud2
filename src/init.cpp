...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-sc2stake=<n>", strprintf(_("Enable or disable staking functionality for SC2 inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zsc2stake=<n>", strprintf(_("Enable or disable staking functionality for zSC2 inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-precompute=<n>", strprintf(_("Enable precomputation of zSC2 spends and stakes (0-1, default %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-zsc2backuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSC2 backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
        ...
        do {
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false) || reindexZerocoin) {
                    ...
                    RecalculateSC2Supply(reindexZerocoin ? Params().Zerocoin_StartHeight() : 1);
                }
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzSC2Wallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSC2Wallet
        uiInterface.InitMessage(_("Syncing zSC2 wallet..."));
    ...
    /* Denominations
       ...
       For example:
       1SC2+1000 == (.1SC2+100)*10
       10SC2+10000 == (1SC2+1000)*10
    */
    ...
    if (pwalletMain) {
        ...
        if (GetBoolArg("-precompute", false)) {
            // Run a thread to precompute any zSC2 spends
