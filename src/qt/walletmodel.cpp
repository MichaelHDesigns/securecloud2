...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered securecloud2 address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal securecloud2:URI (securecloud2:XyZ...?message=example)
            {
...
static void NotifyzSCNReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzSCNReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzSCNReset.connect(boost::bind(NotifyzSCNReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzSCNReset.disconnect(boost::bind(NotifyzSCNReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zsc2Tracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
