...
#define URI_SCHEME "securecloud2"
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SecureCloud2 address (e.g. %1)").arg("S7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or is no SecureCloud2: URI
...
{
    // Convert securecloud2:// to securecloud2:
    //
    //    Cannot handle this later, because securecloud2:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open securecloud2.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SecureCloud2.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SecureCloud2.lnk
...
{
    return GetAutostartDir() / "securecloud2.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a securecloud2.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SecureCloud2\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the securecloud2 app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add securecloud2 app to startup item list
